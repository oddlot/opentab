...
import io.oddlot.opentab.viewmodels.TransactionViewModel
...
/**
 * Activity class for staging/editing new and existing transactions.
 *
 * Notes:
...
//                }
                ...
                txnParcelable?.let {
                    ...
                    xferTypeSwitch.isChecked = isTransfer
                }
                xferTypeSwitch.setOnCheckedChangeListener { btn, checked ->
                    Log.d(TAG, "Switch checked! $checked")
                    isTransfer = checked

                    paidByLabel.text = if (isTransfer) {
                        "Transferred By"
                    } else {
                        "Owed To"
                    }
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        return when (item.itemId) {
            ...
            0 -> { // Submit
                ...
                if (amountPaid.text.isBlank() || amountPaid.text.toString().toInt() == 0) {
