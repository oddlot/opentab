        ...
        CoroutineScope(IO).launch {
            ...
            withContext(Main) {
                if (txn.amount > 0) {
                    ...
                    tvAmount.setTextColor(ContextCompat.getColor(holder.view.context, R.color.AppTheme))
                } else {
...
//        txnDateView.text = StringUtils.dateStringFromMillis(txnDateInMillis, "MM/dd")
        txnDateView.text = StringUtils.dateStringFromMillis(txnDateInMillis, "d MMM")
            ...
            setItems(arr) { dialog, pos ->
                if (pos == 0) {
                    ...
                    thread {
                        val tab = db.tabDao().getTabById(txn.tabId)
                        tabParcelable = TabParcelable(tab.id!!, tab.name, tab.currency)
                        val intent = Intent(context, if (txn.isPayment) PaymentActivity::class.java else TransactionActivity::class.java).apply {
                            putExtra("TAB_PARCELABLE", tabParcelable)
                            putExtra("TXN_PARCELABLE", txnParcelable)
                        }

                        val options = ActivityOptions.makeCustomAnimation(context, R.anim.enter_from_right, R.anim.exit_static).toBundle()
                    ...
                    builder.setTitle("${txn.description}")
