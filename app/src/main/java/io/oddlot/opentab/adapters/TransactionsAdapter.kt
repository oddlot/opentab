...
import io.oddlot.opentab.activities.TabActivity
import io.oddlot.opentab.activities.TransactionActivity
...
//        }
        ...
        CoroutineScope(IO).launch {
            ...
            withContext(Main) {
                if (txn.amount > 0) {
                    tvSummary.text = if (txn.isTransfer) { "You transferred $tabName" } else { "$tabName owes" }
    ...
    private fun longClickListener(context: Context, txn: Transaction): Boolean {
        ...
        builder.setMessage("Are you sure you wish to delete this transaction?\n\n$txn")
